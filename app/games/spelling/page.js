...
export default function SpellingGamePage() {
  ...
  useEffect(() => {
    console.log('🎵 Sound hook check:', { playSound: !!playSound });
    ...
    if (userInput.trim().toLowerCase() === currentWord.word.toLowerCase()) {
      ...
      setTimeout(() => {
        ...
        if (currentIndex < questions.length - 1) {
          // ไปคำถัดไป
          playSound('click'); // ✅ เสียงไปข้อต่อไป
          ...
          playSound('achievement'); // ✅ เสียงพิเศษเมื่อจบเกม
  ...
  const handleHint = () => {
    playSound('click'); // ✅ เสียงคลิกดูคำใบ้
    setShowHint(true);
  };
  ...
  const playAgain = () => {
    playSound('click'); // ✅ เสียงคลิกเล่นอีกครั้ง
    setGameStarted(true);
    setShowResult(false);
  };
  ...
  // ปุ่มทดสอบเสียง (สำหรับ development)
